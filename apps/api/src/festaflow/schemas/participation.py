...
class PublicFestival(BaseModel):
    ...
    # ── 관객 화면의 하단 탭이 이 둘을 본다 ──
    #
    # 특강도 전시도 없는 축제가 대부분입니다(교내 행사에만 붙습니다). 탭을
    # 늘 띄우면 눌러도 "아직 없습니다" 만 나오고, **죽은 링크가 있는 메뉴는
    # 없는 메뉴보다 나쁩니다.**
    #
    # 개수가 아니라 있고 없음만 냅니다 — 참여 전 화면이라 인증이 없고, 몇
    # 개인지는 관객이 그 탭을 열면 알게 됩니다.
    has_lectures: bool = False
